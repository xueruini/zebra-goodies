...
%
% Counts only converge once generated files from the previous pass
% (.aux signatures, .toc replays, etc.) are read back.  The first cold
% pass writes a marker and deliberately skips assertions; later passes
% enforce them.
\makeatletter
\AtBeginDocument{%
  \protected@write\@auxout{}%
    {\string\gdef\string\ZebraTestPreviousRun{1}}%
}
\makeatother
\ExplSyntaxOn
...
\cs_new_protected:Npn \__zebra_test_assert:nnn #1#2#3
  {
    \cs_if_exist:NT \ZebraTestPreviousRun
      {
