TARGET = zebra
OUTPUT = out
DEMO   = $(TARGET)-demo-twocol

all:
	latexmk $(TARGET).dtx

# Package for CTAN upload
ctan: all
	rm -rf $(TARGET) $(TARGET).zip
	mkdir -p $(TARGET)
	cp README.md $(TARGET).dtx $(OUTPUT)/$(TARGET).ins $(OUTPUT)/$(TARGET).pdf $(TARGET)/
	zip -r $(TARGET).zip $(TARGET)
	rm -rf $(TARGET)

# Build and open the PDF viewer
view: all
	latexmk -pv $(TARGET).dtx

clean:
	rm -rf $(OUTPUT)
	rm -f $(TARGET).sty $(TARGET)-goodies.sty $(TARGET).ins $(DEMO).*
	rm -f $(TARGET).aux $(TARGET).glo $(TARGET).gls $(TARGET).idx $(TARGET).ilg $(TARGET).log $(TARGET).out $(TARGET).toc $(TARGET).tmp $(TARGET).hd $(TARGET).dvi

# Run the behavioural regression suite.  The cross-platform runner
# (tests/runtests.lua) compiles each tests/*.tex three times and scans
# the final pass for failure signatures; the article-class tests assert
# their own note counts via tests/zebra-test.tex.  Depends on `all` so a
# fresh out/zebra.sty is built whenever the .dtx changes.
test: all
	texlua tests/runtests.lua

dist-clean: clean
	rm -rf $(TARGET) $(TARGET).zip

.PHONY: all view ctan clean dist-clean test
